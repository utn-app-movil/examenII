...
class alf_DBHelper(context: Context) :
    ...
    fun addContact(contact: Alf_Contact): Long {
    ...
    fun modifyContact(contact: Alf_Contact): Int {
        val db = writableDatabase
        return try {
            val values = ContentValues().apply {
                put(COL_FIRST_NAME, contact.Name)
    ...
    fun fetchContactById(contactId: String): Alf_Contact? {
    ...
    fun fetchAllContacts(): List<Alf_Contact> {
        val db = readableDatabase
        val cursor = db.query(
            TABLE_NAME, null, null, null,
            null, null, null
        )
        val contacts = mutableListOf<Alf_Contact>()
        try {
    ...
    private fun extractContactFromCursor(cursor: android.database.Cursor): Alf_Contact {
        ...
        return Alf_Contact(id, name, lastName, phone, email, residence, nationality, imageBitmap)
    }
